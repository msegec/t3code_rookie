...
it.layer(NodeServices.layer)("manual server handoff", (it) => {
  ...
  it.effect("ignores vanished processes but rejects inaccessible database owners", () =>
    Effect.gen(function* () {
      const { child, dbPath, supported } = yield* makeServer();
      if (!supported) return;
      const originalReadFile = NodeFSP.readFile;
      ...
      const readFile = vi
        ...
        .mockImplementation((...args) =>
          args[0] === `/proc/${child.pid}/cmdline`
            ? Promise.reject(Object.assign(new Error("Process inspection failed"), { code }))
            : originalReadFile(...args),
        );
      try {
        yield* Effect.promise(() => verifyDatabaseOwner(dbPath, undefined));
        code = "EACCES";
        yield* Effect.promise(() =>
          expect(verifyDatabaseOwner(dbPath, undefined)).rejects.toMatchObject({ code }),
        );
      } finally {
        readFile.mockRestore();
      }
