...
async function verifyStandaloneServer(pid: number): Promise<void> {
  ...
  const legacyEntry = args[1]?.endsWith("/t3/dist/bin.mjs") === true;
  if (!legacyEntry) {
    const executablePath = `/proc/${pid}/exe`;
    const executable = await NodeFSP.readlink(executablePath);
    const entry = args[0];
    if (entry === undefined || entry === "" || NodePath.basename(executable) !== "t3") {
      throw new Error("Automatic replacement requires a standalone installed T3 server.");
    }
    const cwd = `/proc/${pid}/cwd`;
    let candidates = [NodePath.resolve(cwd, entry)];
    if (!entry.includes("/")) {
      const environment = (await NodeFSP.readFile(`/proc/${pid}/environ`, "utf8")).split("\0");
      const searchPath = environment.find((value) => value.startsWith("PATH="))?.slice(5);
      const directories = searchPath?.split(":") ?? [];
      if (directories.length > 256) {
        throw new Error("Too many executable paths to verify replacement safely.");
      }
      candidates = directories.map((directory) => NodePath.resolve(cwd, directory, entry));
    }
    const actual = await NodeFSP.stat(executablePath);
    let matches = false;
    for (const candidate of candidates) {
      try {
        const invoked = await NodeFSP.stat(candidate);
        matches = actual.dev === invoked.dev && actual.ino === invoked.ino;
        break;
      } catch (cause) {
        if (
          !(
  ...
  const command = args[legacyEntry ? 2 : 1];
